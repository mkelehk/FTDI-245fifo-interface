...
**245fifo 模式** 是 FTDI USB 芯片常见的一种高速字节流传输模式。该库将 245fifo 封装成接口IP核，留出精简易用的接口，方便Verilog开发者使用。并提供了几个测试用的python程序。

![Image text](https://github.com/WangXuan95/FTDI-245fifo-interface/blob/master/doc/structure.png)

如上图，该IP核解决了以下关键性问题：

* **读写分离**：FTDI 芯片的并口是半双工的，连接到FPGA上为 inout 信号，本IP核将读写接口分离出来，内部使用状态机控制分时复用的读写。
* **跨时钟域**：FTDI 芯片有自己的时钟，与 FPGA 时钟之间有跨时钟域问题。我使用异步 fifo 解决了它。如上图，IP核跨3个时钟域：**读时钟域(淡绿色)**，**写时钟域(淡蓝色)**、**USB时钟域(淡红色)**。不过，读写时钟域可以使用同一时钟。
* **位宽变换**：由于USB芯片引脚数量固定，所以每次能够读写的位宽也是固定的。但开发者可能希望一个时钟周期发送自定义位宽的数据。该IP核的读接口和写接口的位宽可分别自定义。
* **移植性**：纯 SystemVerilog 编写，不调用任何 IP 核，方便在Altera、Xilinx等各种平台上移植

### 测试结果

* **FT232H USB2.0**: 上行链路稳定工作在 **40MBps** 不丢失字节。(上行链路是指 FPGA->USB->PC机器)
* **FT600  USB3.0**: 上行链路稳定工作在 **110MBps** 不丢失字节。(待改进，希望能达到之前裸测的170MBps)
* 上行传输的同时支持少量的下行传输。还没有测试饱和的下行传输带宽。
...
下图是 IP 核的接口图，IP核顶层文件是 **./RTL/src/ftdi_245fifo.sv** ，它还调用了 **./RTL/src/fifo/** 目录下的 4 个 .sv 源文件，除此之外没有其它依赖。

![Image text](https://github.com/WangXuan95/FTDI-245fifo-interface/blob/master/doc/ports.png)

右侧是 FT232H/FT600 等芯片的接口，直接将IP核与芯片引脚按对应名称相连即可。 注意，在有些芯片手册上，例如 FT232H 手册(见./doc/FT232H.pdf第7页)，并不会直接告诉你芯片引脚与本IP核的对应关系，因为FTDI芯片有很多配置模式，只有在 245 sync fifo 模式下才能使用该 IP核，具体的引脚对应请见 ./doc/FT232H.pdf 第8页的表格。

值得注意的是，有两个引脚在该IP核中未出现：**SIWU** 和 **PWRSAV**。如果这两个引脚连接到了FPGA，请将他们置1。另外，**usb_be** 信号是 **usb_data** 的 **byteenable** (独热码)，**usb_be** 只在 FT600 和 FT601 中出现，使用其它芯片时请忽略。
...
| --------         | -----:    | :----: | :----: | :----: | :----: |
| USER_WRITE_DSIZE | 8        | 写接口的数据位宽(wr_data的位宽)，用户自由调整，必须是 8 的 2^n 倍数 | 8,16,32,64等 | 8,16,32,64等 | 8,16,32,64等 |
| USER_READ_DSIZE  |  8    | 读接口的数据位宽(rd_data的位宽)，用户自由调整，必须是 8 的 2^n 倍数 | 8,16,32,64等 | 8,16,32,64等 | 8,16,32,64等 |
